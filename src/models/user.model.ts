...
export interface UserModel extends User, Address, Document {
    getFullName(): string;
    comparePasswords(password: string, encrypted_password: string): Promise<boolean>;
};
...
UserSchema.methods.comparePasswords = async (password: string, encrypted_password: string) => {
    return await Bcrypt.compareSync(password, encrypted_password);
};
